...
function generateProject(project, specRoot, autoRestVersion, cb) {
  ...
  console.log(`> Generating project: "${project}"\n> from spec file: "${specRoot + '/' + mappings[project].source}"\n> with language: "${language}"\n> and AutoRest version: "${autoRestVersion}".`);
...
function codegen(project, index, cb) {
  ...
  if (index !== null && index === 0) {
    ...
    if (!versionSuccessfullyNotFound) {
      process.exit(1);
    }
  }
  return generateProject(project, specRoot, usingAutoRestVersion, cb);
}
