...
StorageAccounts.prototype.checkNameAvailability = function (name, options, callbackOrPromise) {
  var client = this.client;
  if (!callbackOrPromise && typeof options === 'function') {
    callbackOrPromise = options;
    options = null;
  }
  if (!callbackOrPromise) {
    //we have a promise
    return Promise.resolve(this._checkNameAvailability(name, options, function (err, result, request, response) {
      if (err) {
    ...
    return this._checkNameAvailability(name, options, callbackOrPromise);
  }
};
 ...
 */
StorageAccounts.prototype._checkNameAvailability = function (name, options, callback) {
  var client = this.client;
